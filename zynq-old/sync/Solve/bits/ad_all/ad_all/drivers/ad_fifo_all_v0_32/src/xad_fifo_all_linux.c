...
typedef struct {
    ...
    XAd_fifo_all_uio_map maps[ MAX_UIO_MAPS ];
} XAd_fifo_all_uio_info;

/***************** Variable Definitions **************************************/
static XAd_fifo_all_uio_info uio_info;
...
static int uio_info_read_name(XAd_fifo_all_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XAd_fifo_all_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XAd_fifo_all_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XAd_fifo_all_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XAd_fifo_all_Initialize(XAd_fifo_all *InstancePtr, const char* InstanceName) {
	XAd_fifo_all_uio_info *InfoPtr = &uio_info;
