...
typedef struct {
    ...
    XBram2da_start_uio_map maps[ MAX_UIO_MAPS ];
} XBram2da_start_uio_info;

/***************** Variable Definitions **************************************/
static XBram2da_start_uio_info uio_info;
...
static int uio_info_read_name(XBram2da_start_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XBram2da_start_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XBram2da_start_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XBram2da_start_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XBram2da_start_Initialize(XBram2da_start *InstancePtr, const char* InstanceName) {
	XBram2da_start_uio_info *InfoPtr = &uio_info;
