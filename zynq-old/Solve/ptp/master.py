...
def main(addr=ADDRESS):
    ...
    return sync_clock(addr)


def sync_clock(addr=ADDRESS):
    print("\nSyncing time with " + addr + ":" + str(PORT) + " ...")
    send("sync")
    t, resp = recv()
    ...
    if(resp == "ready"):
        ...
        print("\nDone!")
        return (sum(OFFSETS) * ONEBILLION / len(OFFSETS), sum(DELAYS) * ONEBILLION / len(DELAYS))
    else:
